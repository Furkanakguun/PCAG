...
class player:
    ...
    def draw(self):
        if player_moving == LEFT:
            pygame.draw.polygon(screen, black, [(self.x - self.step, self.y),
            ...
            pygame.draw.polygon(screen, black, [(self.x, self.y - self.step),
                                                (self.x + self.step, self.y + self.step),
                                                (self.x - self.step, self.y + self.step)])
        elif player_moving == DOWN:
            pygame.draw.polygon(screen, black, [(self.x, self.y+10),
                                                (self.x + self.step, self.y - self.step),
                                                (self.x - self.step, self.y - self.step)])
        else:
            pygame.draw.rect(screen, black, pygame.Rect(self.x-10,
                                                        self.y-10, 20, 20), 6)
...
player = player(screenSize[0]/2, screenSize[1]/2, 1, 1)
collidables = []
clock = pygame.time.Clock()
for i in range(10):
    collidables.append(collidable(random.randrange(0, screenSize[0]),
                                  random.randrange(0, screenSize[1]),
